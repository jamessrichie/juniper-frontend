...
const sendProfilePictureToApi = async (image) => {
  ...
  try {
    ...
    const response = await fetch(global.API_HOST + "/user/update-profile-pic", {
      method: "POST",
      headers: {
...
const sendFormToApi = async (navigation, values, image, date) => {
...
function PersonalizationScreen({ navigation }) {
      ...
      >
        <Formik
          ...
          onSubmit={async (values) =>
            await sendFormToApi(navigation, values, image, displayedDate)
          }
        >
          {({ handleSubmit }) => (
            <>
